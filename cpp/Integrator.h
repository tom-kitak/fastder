...
class Integrator
{
    ...
    void stitch_up(std::unordered_map<std::string, std::vector<BedGraphRow>>& expressed_regions, const std::map<uint64_t, unsigned int>& mm_sj_counts, const std::vector<SJRow>& rr_all_sj, std::vector<std::string>& chromosome_sequence);
